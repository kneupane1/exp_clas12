...
void Reaction::SetNeutron(float px, float py, float pz,
                          float mass) { // maile thapeko
  _numNeutral++;
  _hasNeutron = true;
...
void Reaction::thrownCalc() {
  ...
  *_pip_mu_prime_cm_thrown =
      physics::boost_(*_pip_thrown, *_beam, *_elec_thrown);
  // *_q_cm = physics::boost_((*_beam - *_elec), *_beam, *_elec);
  *_p_mu_prime_cm_thrown =
      physics::boost_(*_prot_thrown, *_beam, *_elec_thrown);
  *_pim_mu_prime_cm_thrown =
      physics::boost_(*_pim_thrown, *_beam, *_elec_thrown);
  //}
}
...
TLorentzVector Reaction::p_mu_prime_thrown() { return *_prot_thrown; }
TLorentzVector Reaction::pip_mu_prime_thrown() { return *_pip_thrown; }
TLorentzVector Reaction::pim_mu_prime_thrown() { return *_pim_thrown; }
...
float Reaction::alpha_pippim_pipf() { return _alpha_pippim_pipf; }
float Reaction::alpha_ppim_pipip() { return _alpha_ppim_pipip; }
...
void Reaction::AlphaCalc_thrown() {
  ...
  *_q_cm_thrown =
      (physics::boost_((*_beam - *_elec_thrown), *_beam, *_elec_thrown));
  // *_p_mu_prime_cm = physics::boost_(*_prot, *_beam, *_elec);
  // *_pip_mu_prime_cm = physics::boost_(*_pip, *_beam,*_elec);
  // *_pim_mu_prime_cm = physics::boost_(*_pim, *_beam, *_elec);
  ...
  // 1 this one is used for α[π−]
  a_gamma_thrown = sqrt(
      1. / (1 - pow((_pim_mu_prime_cm_thrown_.Vect().Unit() * V3_anti_z_thrown),
                    2))); // V3_anti_z_thrown(0,0,-1);
  b_gamma_thrown =
      -(_pim_mu_prime_cm_thrown_.Vect().Unit() * V3_anti_z_thrown) *
      a_gamma_thrown;
  Vect3_gamma_thrown = a_gamma_thrown * V3_anti_z_thrown +
                       b_gamma_thrown * _pim_mu_prime_cm_thrown_.Vect().Unit();

  a_beta_thrown = sqrt(1. / (1 - pow((_pim_mu_prime_cm_thrown_.Vect().Unit() *
                                      _pip_mu_prime_cm_thrown_.Vect().Unit()),
                                     2)));
  b_beta_thrown = -(_pim_mu_prime_cm_thrown_.Vect().Unit() *
                    _pip_mu_prime_cm_thrown_.Vect().Unit()) *
                  a_beta_thrown;
  Vect3_beta_thrown = a_beta_thrown * _pip_mu_prime_cm_thrown_.Vect().Unit() +
                      b_beta_thrown * _pim_mu_prime_cm_thrown_.Vect().Unit();

  alpha_PPIp_piPIm_thrown =
      (180. / PI) * acos(Vect3_gamma_thrown * Vect3_beta_thrown);
  if (Vect3_gamma_thrown.Cross(Vect3_beta_thrown) *
          _pim_mu_prime_cm_thrown_.Vect() <
      0)
  ...
  a_gamma_thrown = sqrt(
      1. /
      (1 - pow((_p_mu_prime_cm_thrown_.Vect().Unit() * V3_anti_z_thrown), 2)));
  b_gamma_thrown = -(_p_mu_prime_cm_thrown_.Vect().Unit() * V3_anti_z_thrown) *
                   a_gamma_thrown;
  Vect3_gamma_thrown = a_gamma_thrown * V3_anti_z_thrown +
                       b_gamma_thrown * _p_mu_prime_cm_thrown_.Vect().Unit();

  a_beta_thrown = sqrt(1. / (1 - pow((_p_mu_prime_cm_thrown_.Vect().Unit() *
                                      _pip_mu_prime_cm_thrown_.Vect().Unit()),
                                     2)));
  b_beta_thrown = -(_p_mu_prime_cm_thrown_.Vect().Unit() *
                    _pip_mu_prime_cm_thrown_.Vect().Unit()) *
                  a_beta_thrown;
  Vect3_beta_thrown = a_beta_thrown * _pip_mu_prime_cm_thrown_.Vect().Unit() +
                      b_beta_thrown * _p_mu_prime_cm_thrown_.Vect().Unit();

  alpha_PIpPIm_pipf_thrown =
      (180. / PI) * acos(Vect3_gamma_thrown * Vect3_beta_thrown);

  if (Vect3_gamma_thrown.Cross(Vect3_beta_thrown) *
          _p_mu_prime_cm_thrown_.Vect() <
      0)
  ...
  /// 3
  a_gamma_thrown = sqrt(
      1. / (1 - pow((_pip_mu_prime_cm_thrown_.Vect().Unit() * V3_anti_z_thrown),
                    2)));
  b_gamma_thrown =
      -(_pip_mu_prime_cm_thrown_.Vect().Unit() * V3_anti_z_thrown) *
      a_gamma_thrown;
  Vect3_gamma_thrown = a_gamma_thrown * V3_anti_z_thrown +
                       b_gamma_thrown * _pip_mu_prime_cm_thrown_.Vect().Unit();

  a_beta_thrown = sqrt(1. / (1 - pow((_pip_mu_prime_cm_thrown_.Vect().Unit() *
                                      _pim_mu_prime_cm_thrown_.Vect().Unit()),
                                     2)));
  b_beta_thrown = -(_pip_mu_prime_cm_thrown_.Vect().Unit() *
                    _pim_mu_prime_cm_thrown_.Vect().Unit()) *
                  a_beta_thrown;
  Vect3_beta_thrown = a_beta_thrown * _pim_mu_prime_cm_thrown_.Vect().Unit() +
                      b_beta_thrown * _pip_mu_prime_cm_thrown_.Vect().Unit();

  alpha_PPIm_piPIp_thrown =
      (180. / PI) * acos(Vect3_gamma_thrown * Vect3_beta_thrown);

  if (Vect3_gamma_thrown.Cross(Vect3_beta_thrown) *
          _pip_mu_prime_cm_thrown_.Vect() <
      0)
...
float Reaction::alpha_ppip_pipim_thrown() { // pipim bhaneko proton prime pim
                                            // ho?
  return _alpha_ppip_pipim_thrown;
}
float Reaction::alpha_pippim_pipf_thrown() { return _alpha_pippim_pipf_thrown; }
float Reaction::alpha_ppim_pipip_thrown() { return _alpha_ppim_pipip_thrown; }
...
float Reaction::W_thrown() { return _W_thrown; }
float Reaction::Q2_thrown() { return _Q2_thrown; }
...
float Reaction::W_2pi_thrown() { return _W_2pi_thrown; }
float Reaction::W_delta_pp_thrown() { return _W_delta_pp_thrown; }
float Reaction::W_delta_zero_thrown() { return _W_delta_zero_thrown; }
float Reaction::W_rho_thrown() { return _W_rho_thrown; }
